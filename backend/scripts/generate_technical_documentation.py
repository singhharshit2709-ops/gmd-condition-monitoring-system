#!/usr/bin/env python3
"""Generate GMD technical documentation package (Markdown + Word-ready + Executive Summary)."""
...
EXPORT = REPORTS / "config_hierarchy_export.json"
...
def area_stats(hierarchy: dict) -> list[tuple[str, int, list[str]]]:
    rows = []
    for bucket, cats in hierarchy.items():
        total = sum(len(v) for v in cats.values())
        rows.append((bucket, total, sorted(cats.keys())))
    return rows


def build_main_documentation(data: dict) -> str:
    hierarchy = data["hierarchy"]
    stats = area_stats(hierarchy)
    total_equipment = sum(n for _, n, _ in stats)
    unique_keys = len({p["key"] for p in data["parameter_catalogue"]})
    param_instances = len(data["parameter_catalogue"])

    stat_lines = "\n".join(
        f"| {area} | {count} | {', '.join(cats)} |"
        for area, count, cats in stats
    )

    return f"""# GMD Condition Monitoring System — Technical Documentation

**Document version:** 1.0  
**Generated:** {TODAY}  
**System:** Neutral Glass — General Maintenance Department (GMD) Electrical Condition Monitoring  
**Configuration:** `gmd_machine_config_v2.json` — {total_equipment} equipment instances, {param_instances} parameter slots, {unique_keys} unique parameter keys
...
- `PARAMETER_EQUIPMENT_MATRIX.md` — all {param_instances} parameter-to-equipment mappings
...
The GMD Condition Monitoring System is a web application for daily engineering rounds at a glass manufacturing plant. Operators record vibration, temperature, pressure, water quality, and electrical readings for **{total_equipment} configured equipment instances** across six dashboard areas. Data persists to **Google Sheets** (canonical row store) with optional **Google Drive** media attachments. The **React** frontend provides Dashboard, Add Reading, Equipment Monitoring, Reports, and Trends & Analytics modules. The **FastAPI** backend serves REST APIs, validates submissions against JSON configuration, and integrates with Google APIs.
...
**Total configured equipment instances:** {total_equipment}
...
See **`backend/reports/PLANT_HIERARCHY_APPENDIX.md`** for every equipment name, category, tag, and parameter list.

---

# Part 5: Parameter Documentation

## 5.1 Unique Parameter Keys ({unique_keys} keys)
...
See **`backend/reports/PARAMETER_CATALOGUE.md`** for the master key table.
...
See **`backend/reports/PARAMETER_EQUIPMENT_MATRIX.md`** ({param_instances} rows) for Parameter Key × Equipment × Tank/Area × Tag mappings.
...
- ✅ Dynamic V2 configuration ({total_equipment} equipment)
...
Suitable for current scale (~{total_equipment} equipment, daily rounds). Sheets API rate limits and full-sheet reads will degrade with years of data. Recommend archival worksheet strategy or external time-series DB before 100k+ rows.
...
*End of Technical Documentation — Generated {TODAY}*
"""


def build_word_ready(main: str) -> str:
    """Word-friendly variant: explicit title page, simplified tables note."""
    header = f"""---
...
"""
    # Replace mermaid-unfriendly code blocks with plain text for Word
    return header + main.replace("```", "\n```\n")
...
def build_executive_summary(data: dict) -> str:
    ...
    return f"""# GMD Condition Monitoring System — Executive Summary
...
**Plant coverage:** {total_equipment} equipment instances, {unique_keys} parameter types, 6 dashboard areas.
...
def main() -> None:
    ...
    main_doc = build_main_documentation(data)

    hierarchy_appendix = (REPORTS / "PLANT_HIERARCHY_APPENDIX.md").read_text(encoding="utf-8")
    param_catalogue = (REPORTS / "PARAMETER_CATALOGUE.md").read_text(encoding="utf-8")

    # Embed full hierarchy and parameter catalogue into main doc (Part 4 & 5)
    main_doc = main_doc.replace(
        "See **`backend/reports/PLANT_HIERARCHY_APPENDIX.md`** for every equipment name, category, tag, and parameter list.",
        "### 4.7 Complete Equipment Listing\n\n" + hierarchy_appendix,
    )
    main_doc = main_doc.replace(
        "See **`backend/reports/PARAMETER_CATALOGUE.md`** for the master key table.",
        param_catalogue.replace("# Master Parameter Key Catalogue", "### 5.1.1 Master Parameter Key Catalogue"),
    )

    word_doc = build_word_ready(main_doc)
    exec_doc = build_executive_summary(data)

    (REPORTS / "GMD_TECHNICAL_DOCUMENTATION.md").write_text(main_doc, encoding="utf-8")
    (REPORTS / "GMD_TECHNICAL_DOCUMENTATION_WORD.md").write_text(word_doc, encoding="utf-8")
    (REPORTS / "GMD_EXECUTIVE_SUMMARY.md").write_text(exec_doc, encoding="utf-8")

    print("Wrote:")
    for name in [
        "GMD_TECHNICAL_DOCUMENTATION.md",
        "GMD_TECHNICAL_DOCUMENTATION_WORD.md",
        "GMD_EXECUTIVE_SUMMARY.md",
    ]:
        path = REPORTS / name
